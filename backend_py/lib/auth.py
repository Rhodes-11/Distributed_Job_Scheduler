...
async def issue_refresh_token(user_id: str, meta: dict[str, str | None]) -> tuple[str, datetime]:
...
async def rotate_refresh_token(old_token: str, meta: dict[str, str | None]) -> tuple[str, datetime, str]:
    try:
    ...
    await db.refresh_tokens.update_one({'_id': stored['_id']}, {'$set': {'revoked_at': datetime.utcnow()}})
    fresh, expires_at = await issue_refresh_token(payload['sub'], meta)
...
async def revoke_refresh_token(token: str) -> None:
    token_hash = hashlib.sha256(token.encode()).hexdigest()
    db = get_db()
    await db.refresh_tokens.update_many({'token_hash': token_hash, 'revoked_at': None}, {'$set': {'revoked_at': datetime.utcnow()}})
